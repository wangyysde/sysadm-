	...
	**/
	...
	public function price_save($tid,$data=array(),$vtype="price")
	{
		...
		if($tmplist){
			...
			if($adds){
				foreach($adds as $key=>$value){
					$tmp = array('region_id'=>$value,'tid'=>$tid,'price'=>$data[$value],'vtype'=>$vtype);
					$this->db->insert($tmp,'world_price');
				}
			}
			if($updates){
				foreach( $updates as $key => $value ){
					$sql = "UPDATE ".$this->db->prefix."world_price SET price='".$data[$value]."' WHERE tid='".$tid."' AND vtype='".$vtype."' AND region_id='".$value."'";
					$this->db->query($sql);
				}
			}
			return true;
		...
		foreach($data as $key=>$value){
			$tmp = array('region_id'=>$key,'tid'=>$tid,'price'=>$value,'vtype'=>$vtype);
			$this->db->insert($tmp,'world_price');
		}
		return true;
	}
