...
class install
{
	public function head($num=1)
	{
		...
		echo '<title>'.SYSNAME.'企业站系统安装</title>'."\n";
		$this->cssinfo();
		$this->jsinfo();
		echo '</head>'."\n";
		echo '<body>'."\n";
		echo '<div class="header">';
		echo '<div class="logo">'.SYSNAME.' <span>'.VERSION.'</span></div>';
	...
	{
		echo '<div class="footer">Powered By '.OFFICEURL.' 版权所有 &copy; '.COPYSTART.'-'.date("Y").', All right reserved.</div>'."\n";
		echo '</body>'."\n";
		echo '</html>';
		exit;
	}
	...
	public function env_check()
	{
		...
		echo '<tr class="head_bg"><td>&nbsp;</td><td>'.SYSNAME.'最低要求</td><td>'.SYSNAME.'最佳配置</td><td>当前环境检测</td></tr>';
			...
			if(info){
				if(info == 'ok'){
					...
					if(id == 'endok'){
						...
						alert("安装完成，您可以点击\"进入后台\"按钮可以进入后台设置");
					}else{
						var _i = 0;
