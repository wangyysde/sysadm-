...
class alipay_submit
{
	...
	public function __construct($order,$param)
	{
		...
		//新版写法，目前主要是基于App进行调试
        include_once($this->paydir."aop/AopClient.php");
		include_once $this->paydir."aop/AopCertification.php";
		include_once $this->paydir."aop/request/AlipayTradeQueryRequest.php";
		include_once $this->paydir."aop/request/AlipayTradeWapPayRequest.php";
		include_once $this->paydir."aop/request/AlipayTradeAppPayRequest.php";
	}
	...
	function submit($json=false)
	{
        ...
        if($this->param['param']['ptype'] == 'app' && $json){
			...
			$tmpdata = array();
			$tmpdata['subject'] = $this->order['title'];
			$tmpdata['body'] = $this->order['content'];
			$tmpdata['out_trade_no'] = $this->order['sn'].'-'.$this->order['id'];
			$tmpdata['total_amount'] = floatval($total_fee);
			$tmpdata['product_code'] = 'QUICK_MSECURITY_PAY';
			$bizcontent = $app->lib('json')->encode($tmpdata);
			$request->setNotifyUrl($notify_url);
			$request->setBizContent($bizcontent);
			$response = $aop->sdkExecute($request);
			$app->success(array('orderInfo'=>$response,'provider'=>'alipay'));
        }
		$parameter = array(
